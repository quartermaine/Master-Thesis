...
from datasets import flip_vol

'''
This script combines the requsted derivatives and returns TfRecords
splitted to train, valid, test
'''
...
def compute_n_files(nImages):
    # Determine number of TFRecord fileas to save
    maxImagesPerFile = 32
    nImagesPerFile = []
    temp = nImages
    while temp >= 0:
...
def main():
    ...
    "TfRecords": [{'train':f'{nImages_train}','valid':f'{nImages_valid}','test':f'{nImages_test}'}],
